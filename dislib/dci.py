...
Based on "A Framework for the Quantitative Evaluation of Disentangled Representations" (https://openreview.net/forum?id=By-7dz-AZ).
...
class DciMetric:
    def __init__(self, ds, num_points=utils.NUM_POINTS, paired=False):
        """ DCI Metric
    ...
    def __call__(self, model):
        ...
        rep_fn = lambda x: model.encode_image(x.unsqueeze(0))[0]
        mus_train, ys_train = utils.sample_batch(rep_fn, self.num_points, self.ds, paired=self.paired, mode="train")
        assert mus_train.shape[1] == self.num_points
        assert ys_train.shape[1] == self.num_points
        mus_test, ys_test = utils.sample_batch(rep_fn, self.num_points, self.ds, paired=self.paired, mode="test")
